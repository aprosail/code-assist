...
export async function generateInterfaces(
  ...
  const raw = JSON5.parse(readFileSync(template).toString())["colors"]
 ...
 */
export function updateObject(raw: Object, path: string[], value: unknown) {
  ...
  updateObject(raw[path[0]], path.slice(1), value)
}
